...
def modifyJets( topoclAlgs, rsize ):
    from AthenaCommon.AlgSequence import AlgSequence
    topSequence=AlgSequence()
    ...
    getter = PseudoJetGetter("JetGetter" + str(int(rsize*10)) + "_" + nameAffix,
                              Label = Label,
                              InputContainer = nameAffix + "TopoClusters",
                              OutputContainer = "PseudoJets" + str(int(rsize*10)) + nameAffix,
                              SkipNegativeEnergy = True,
                          )
    jtm += getter
    
    addStandardJets("AntiKt", rsize, label, algseq=topSequence, namesuffix=nameAffix, outputGroup="output_"+nameAffix, customGetters=[getter])
