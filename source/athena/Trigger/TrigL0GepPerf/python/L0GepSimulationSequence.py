...
def setupL0GepSimulationSequence(
    ...
    from TrigL0GepPerf.JetBuilder import modifyClusters, modifyJets
    from TrigL0GepPerfConf import GepClusteringAlg
    from TrigL0GepPerfConf import GepJetAlg
    from TrigL0GepPerfConf import ClusterEtCutAlg
    ...
    for topoMaker in topoclAlgs:
        ...
        for puSupAlg in puSupprAlgs:            
            ...
            topoclLabelPUSub = topoMaker+puSupAlg

            # create new clusters containers with given Et cuts
            topSequence += ClusterEtCutAlg( topoclLabelPUSub, TopoclLabel=topoclLabelPUSub, TopoclEtGeVCuts=topoclEtGeVCuts )
            
            for topoEtCut in topoclEtGeVCuts:
                topoclLabel = topoclLabelPUSub + 'Cut' + str(topoEtCut) + 'GeV'
            
                # jets
                for jetAlg in jetAlgs:
                    if jetAlg=='AntiKt4':
                        # run Athena AntiKt4 jet reconstruction
                        modifyJets( topoclLabel, 0.4 )      
                    elif jetAlg=='AntiKt10':
                        modifyJets( topoclLabel, 1.0 )      
                    else:
